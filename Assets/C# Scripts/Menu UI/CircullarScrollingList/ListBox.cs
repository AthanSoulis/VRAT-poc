...
public class ListBox : MonoBehaviour
{
	...
	void Start()
	{
        ...
        /*A.S.*/ _originalLocalScale = transform.localScale;
        //Original:_originalLocalScale = transform.lossyScale;
	...
	void initialContent()
	{
		if (listBoxID == ListPositionCtrl.Instance.listBoxes.Length / 2)
			...
			_contentID = ListBank.Instance.getRealListLength() - (ListPositionCtrl.Instance.listBoxes.Length / 2 - listBoxID);//ListBank.Instance.getListLength() - (ListPositionCtrl.Instance.listBoxes.Length / 2 - listBoxID);
        else
			_contentID = listBoxID - ListPositionCtrl.Instance.listBoxes.Length / 2;

		while (_contentID < 0)
			_contentID += ListBank.Instance.getRealListLength();//ListBank.Instance.getListLength();
        _contentID = _contentID % ListBank.Instance.getRealListLength();//_contentID % ListBank.Instance.getListLength();
	...
	/* Scale the size of listBox according to the position.
	 */
	...
	void updateToLastContent()
	{
        ...
        _contentID = (_contentID < 0) ? ListBank.Instance.getRealListLength() - 1 : _contentID;
	...
	void updateToNextContent()
	{
        ...
        _contentID = (_contentID == ListBank.Instance.getRealListLength()) ? 0 : _contentID;
    ...
    {
        VRATNode node = ListBank.Instance.getListContentFNode(_contentID);
        if (node.type == "GoOn")
            VRATSceneManager.Instance.VRATtraverseGoOnSelection();
        else
            VRATSceneManager.Instance.VRATtraverseBranchSelection(node);
    }
}
