...
public class ListPositionCtrl : MonoBehaviour
{
	 ...
	 */
    //A.S. In order for the changes to occur after enabling/disabling ,
    // the implementation of initialization is moved in OnEnable
    //void Start()
    private void OnEnable()
	{
		...
		_lowerBoundPos_L = _unitPos_L * (-1 * ListBank.Instance.getListLength()/* A.S. listBoxes.Length*/ / 2 - 1);
		_upperBoundPos_L = _unitPos_L * (ListBank.Instance.getListLength()/* A.S. listBoxes.Length*/ / 2 + 1);
		_shiftBoundPos_L = _unitPos_L * 0.3f;

		// If there are even number of ListBoxes, narrow the boundary for 1 unitPos.
		if ((ListBank.Instance.getListLength()/* A.S. listBoxes.Length*/ & 0x1) == 0) {
        ...
        listBoxes = ListBank.Instance.getListBoxArray();
    }

    //A.S. After the List is disabled the ListBank is cleared
    private void OnDisable()
    {
        ListBank.Instance.clearContents();
    }
